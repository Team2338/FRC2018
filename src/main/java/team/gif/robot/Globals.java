package team.gif.robot;

public class Globals {

    // Drivetrain Constants
    public static final double DEFAULT_QUICK_STOP_THRESHOLD = 0.2;
    public static final double DEFAULT_QUICK_STOP_ALPHA = 0.1;
    public static final double MAX_OUTPUT = 1.0;
    public static final double DEFAULT_DEADBAND = 0.02;

    // Arm Constants
    public static final double FRONT_P = 0.0;
    public static final double FRONT_I = 0.0;
    public static final double FRONT_D = 0.0;
    public static final double FRONT_F = 0.0;

    public static final double DART_P = 0.0;
    public static final double DART_I = 0.0;
    public static final double DART_D = 0.0;

    public static final int SPEED_TOLERANCE = 100;
}
